  ...
  try {
    ...
    if (body.sortOrder) {
      const existing = await prisma.project.findFirst({
        where: { sortOrder: parseInt(body.sortOrder) },
      });
      if (existing) {
    ...
    const project = await prisma.project.create({
      data: {
        ...
        statusApartment:
          body.statusApartment !== undefined
            ? parseInt(body.statusApartment)
            : 1,
        availability:
          body.availability !== undefined ? parseInt(body.availability) : 1,
