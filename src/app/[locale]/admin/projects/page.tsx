...
export default function ProjectsListPage() {
  ...
  const [sortError, setSortError] = useState(""); // ← add this
  ...
  const changeSort = async (id: number, value: number) => {
    ...
    if (!res.ok) {
      const data = await res.json();
      setSortError(data.error || "Failed to update order");
    }
  };
                ...
                <tr key={project.id}>
                  ...
                  <td>
                    <input
                      ...
                      onChange={(e) =>
                        changeSort(project.id, parseInt(e.target.value) || 0)
                      }
                    />
                  </td>
                  <td>
